...
% -- 6 --
\subsection*{6.\ Result 4: The Tau-Electron Mass Ratio}
...
\begin{itemize}[nosep,leftmargin=1.5em]
  ...
  \item $\pi^2$ --- BZ momentum integral (same $\pi^2$ as $c_1$ in Result~1)
  \item $\alpha^2$ --- two-loop EM coupling
  \item $1/(n{-}1) = 1/7$ --- spectral variance (same as $c_3$ in Result~1)
  \item $1/\sigma = 1/136$ --- pairwise channel normalization
\end{itemize}

With $m_e = 1$ and $m_\mu$ from Result~3, the Koide equation reduces to a
...
% -- 7 --
\subsection*{7.\ Result 5: The Higgs Boson Mass}
...
% -- 8 --
\subsection*{8.\ Result 6: The Neutron-Proton Mass Difference}
...
\begin{equation*}
  \boxed{\frac{\Delta m}{m_e}
    = \binom{n{-}1}{d}\,\pi^2\alpha\!\left(1 + \frac{(n{-}d)\,\alpha}{\rho}\right)
    + \frac{\rho}{2n}\,\pi\alpha^2 - \alpha^2}
\end{equation*}
...
\begin{itemize}[nosep,leftmargin=1.5em]
  ...
  \item \textbf{VP:} $(\rho/2n)\,\pi\alpha^2 = \tfrac{9}{16}\pi\alpha^2$
    (cross-references Higgs and proton channels). Closes to 0.13\,ppm.
  \item \textbf{Self-intersection:} $-\alpha^2$, universal.
\end{itemize}
...
\begin{tabular}{lc}
\toprule
Lattice framework prediction & $2.531\,030$ \\
Experiment & $2.531\,030(3)$ \\
Agreement & $0.13\,\mathrm{ppm}$ \\
\bottomrule
\end{tabular}
\end{table}
...
% -- 9 --
\subsection*{9.\ The Vacuum Polarization Catalog}

Each BSM mass formula contains exactly one VP correction, governed by a
distinct geometric generator. The four base generators---$\tau$, $2n$, $d$,
$\rho$---are each assigned to exactly one sector (\emph{non-overlap principle}):

\begin{table}[H]
\centering\small
\begin{tabular}{llcl}
\toprule
\textbf{Observable} & \textbf{VP correction} & \textbf{Generator} & \textbf{Structure} \\
\midrule
$\ainv$ & $c_1/\tau = \pi^2/(2\tau)$ & $\tau = 137$ & Topological \\
$m_p/m_e$ & $(1+1/(2n))\pi^2\alpha(1{-}\alpha^2)$ & $2n = 16$ & Coordination \\
$m_\mu/m_e$ & $(d/2)\pi\alpha(1{-}\alpha^2)$ & $d = 3$ & Spatial dimension \\
$m_H/m_p$ & $\pi\alpha/\rho$ & $\rho = 9$ & Radial \\
$\Delta m/m_e$ & $(\rho/2n)\pi\alpha^2$ & $\rho/(2n)$ & Cross-reference \\
\bottomrule
\end{tabular}
\end{table}

\noindent The $\Delta m$ VP uses $\rho/(2n)$, a product of two generators; it
is the only two-loop VP ($\alpha^2$), cross-referencing the Higgs ($\rho$)
and proton ($2n$) channels. This non-overlap principle resolves the Higgs
correction ambiguity: $\pi\alpha/\rho$ is the unique choice that preserves
generator injectivity (the alternatives $\pi\alpha/(2d)$ and $\pi\alpha/n$
reuse generators already assigned to the muon and proton sectors).

% -- 10 --
\subsection*{10.\ Lattice Selection: Why BCC?}
...
% -- 11 --
\subsection*{11.\ Questions for the Reader}
...
\begin{enumerate}[label=(\alph*),nosep,leftmargin=1.5em]
  ...
  \item The VP catalog assigns four generators ($\tau$, $2n$, $d$, $\rho$)
    injectively to four mass sectors. Is such a non-overlap structure
    expected from gauge invariance or a selection rule?
\end{enumerate}
